...
// Recorder writes the FrontendApp metric set. The zero value is fully usable:
...
func (m *Recorder) RecordApp(app *bakerv1alpha1.FrontendApp, deadlineSeconds int64, alertThresholds map[string]int64) {
	m.mu.Lock()
	defer m.mu.Unlock()
	s := m.state(app)
...
func syncUsedSeries(app *bakerv1alpha1.FrontendApp, volume string, sizes map[string]int64) {
