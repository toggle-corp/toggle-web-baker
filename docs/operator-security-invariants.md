...
- **`backoffLimit: 0`** (a failed build fails once → `Degraded`; last-good keeps
  serving). **`activeDeadlineSeconds`** bounds wedged builds: it is derived from
  `spec.pipeline.timeout` (a duration, e.g. `"1h"`) when set, otherwise the
  operator config default (`activeDeadlineSeconds` in the mounted config file).
  The former hardcoded 1800s literal moved into operator config.
- **Every phase container (setup / fetch / build) carries resource requirements
  ...
  ceiling is the app's per-phase `spec.pipeline.phases.<phase>.memoryLimit` when set, else the
